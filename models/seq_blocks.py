...
class MlpNet(nn.Module):
    def __init__(self,
        ...
        if ch_list is None or len(ch_list) == 0:
            ch_list = [in_ch]
            for ind_block in range(1, deep + 1, 1):
                if ind_block == deep and out_ch is not None:
                    curr_ch = out_ch
                else:
                    curr_ch = round(ch_list[ind_block - 1] * multy_coeff)
                ch_list.append(curr_ch)

        for ind_block in range(deep):
            in_, out_ = ch_list[ind_block], ch_list[ind_block + 1]
            fc_block = FcBlock(in_features=in_,
                               out_features=out_,
